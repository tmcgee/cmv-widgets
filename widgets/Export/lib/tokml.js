...
function extendeddata(_, options) {
    return options.extendedData ? tag('ExtendedData', pairs(_).map(data).join('')) : '';
}
...
function hasMarkerStyle(_) {
    return !!(_['marker-size'] || _['marker-symbol'] || _['marker-color'] || _['scale'] || _['href'] || _['label-color'] || _['label-scale']);
}
...
function iconUrl(_) {
    ...
    return href || 'https://api.tiles.mapbox.com/v3/marker/' + 'pin-' + size.charAt(0) +
        symbol + '+' + color + '.png';
}

function iconColor(_) {
    return _['color'] ? tag('color', hexToKmlColor(_['color'], _['marker-opacity'] || 1.0)) : '';
}

function iconScale(_) {
    return (_['scale'] !== undefined) ? tag('scale', _['scale']) : '';
}

function iconSize(_) {
    return tag('hotSpot', '', [
        ['xunits', _['xunits'] || 'fraction'],
        ['yunits', _['yunits'] || 'fraction'],
        ['x', (_['x'] !== undefined) ? _['x'] : 0.5],
        ['y', (_['y'] !== undefined) ? _['yx'] : 0.5]
    ]);
}
...
function polygonAndLineStyle(_, styleHash) {
    ...
    return tag('Style', lineStyle + polyStyle + labelStyle(_), [['id', styleHash]]);
}

// ## Label style
function labelStyle(_) {
    return (_['label-color'] || (_['label-scale'] !== undefined)) ?
        tag('LabelStyle', labelColor(_) + labelScale(_))
        : '';
}

function labelColor(_) {
    return _['label-color'] ? tag('color', hexToKmlColor(_['label-color'], (_['label-opacity'] !== undefined) ? _['label-opacity'] : 1.0)) : '';
}

function labelScale(_) {
    return (_['label-scale'] !== undefined) ? tag('scale', _['label-scale']) : '';
}
...
function hashStyle(_) {
    ...
    if (_['href']) hash = hash + 'mh' + hashString(_['href']);
    if (_['color']) hash = hash + 'mco' + _['color'];
    if (_['scale']) hash = hash + 'msc' + _['scale'];
    if (_['label-color']) hash = hash + 'lco' + _['label-color'];
    if (_['label-scale']) hash = hash + 'lsc' + _['label-scale'];
